...
//		super.init(config);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String pname=su.getRequest().getParameter("pname").trim();
			String pdetail=su.getRequest().getParameter("pdetail").trim();
			String ptest=su.getRequest().getParameter("ptest").trim();
			
			//台式机路径
//			String pathRootplogo="C:\\Users\\AD钙\\git\\WeAdmin\\WeAdmin\\WebContent\\images\\plogo\\";
			
			//平板路径
//			String pathRootplogo="C:\\Users\\11040\\git\\WeAdmin\\WeAdmin\\WebContent\\images\\plogo\\";

			
			//获取上传的路径,指的是tomcat路径下的文件夹,而前台需要的是项目内的文件夹，待改善
			String uploadFile=request.getRealPath("images/plogo/");
			...
			for (int i = 0; i < su.getFiles().getCount(); i++) {
				...
				if(!fileName.equals("")) {
					...
					if(fName.equals(".png")||fName.equals(".jpg")||fName.equals(".gif")) {
						//执行上传
						file.saveAs("images/plogo/"+fileName,SmartUpload.SAVE_VIRTUAL);
						f=true;
					}
				}
			}
...
//				String slogo=uploadFile+"\\"+fileName;
				int count=cm.proRegistration(pname,pdetail,ptest,fileName);
				if(count>0){
					pw.print("ok");
				}else{
					pw.print("err");
